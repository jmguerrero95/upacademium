  ...
  // echo "consulta:".$sqlCmd;
   ...
   $serial_id=0;
	...
	if ($rsAsistencia && $rsAsistencia->RecordCount()>0) {
		...
		//observaciones
		if(!empty($params[6])){
			$msgsalida=" ,observaciones_asi=concat(observaciones_asi,'  / SAL: ','".$params[6]."')";
		}
		//tramite
		if(!empty($params[7])){
			$msgtramite = " ,tramite_asi=concat(tramite_asi,'  / SAL: ','".$params[7]."')";
		}
		//contacto
		if(!empty($params[8])){
			$msgcontacto = " ,contacto_asi=concat(contacto_asi,'  / SAL: ','".$params[8]."')";
		}

		 $sqlCmd="update asistencia set salida_asi='".$params[2]."' ".$msgsalida.$msgtramite.$msgcontacto." where serial_asi=".$rsAsistencia->fields[0];
		 $dblink->Execute($sqlCmd);
		 $serial_id=$rsAsistencia->fields[0];
    }
    else {
     $sqlCmd="insert into asistencia (SERIAL_ASI,SERIAL_EPL, FECHA_ASI, ENTRADA_ASI, SALIDA_ASI, ESTADO_ASI, atraso_asi, permiso_asi, observaciones_asi,tramite_asi,contacto_asi) values (0,";
     $sqlCmd.=$params[0].",'".$params[1]."','".$params[2]."','','".$params[3]."',".$params[4].",0,'".$params[6]."','".$params[7]."','".$params[8]."')";
