...
omnisoftListBox=function(aObjName,alistName,aSQLCommand,sSize,sWidth,aCheck,aValue,aAll,aOnChange) {
 ...
 this.create=function() {
    with (document) {
      if (this.check)
   write('<select size='+this.size+' class="ListBox" name="'+this.listName+'" STYLE="width: '+this.width+'" id="'+this.listName+'"  onKeypress='+this.ObjName+'.search(this) onblur='+this.ObjName+'.empty()>');
   else
   write('<select size='+this.size+' name="'+this.listName+'" STYLE="width: '+this.width+'" id="'+this.listName+'"  onKeypress='+this.ObjName+'.search(this) onblur='+this.ObjName+'.empty()>');
   write(' </select>');
   }
 }
 this.show=function() {
