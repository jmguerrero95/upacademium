...
function consultaConsolidada($area,$facultad){
  ...
  global $DBConnection;
		...
		/********va sacando el Total de todos de los profesores filtrado por area y periodo************/					
					...
					$promedio='select serial_teva, sum( valor_rsp ) as suma, count( DISTINCT ce.serial_ceva ) AS num_eval 
								from cabecera_evaluacion ce, detalle_evaluacion de, horario hrr
...
/********va sacando el Total de de todos profesores filtrado por facultad y periodo************/					
						...
						$promediofacultad='select serial_teva, sum( valor_rsp ) AS suma, count( DISTINCT ce.serial_ceva ) as num_eval
							from cabecera_evaluacion as ce, detalle_evaluacion as de, horario as hrr
							WHERE de.serial_ceva = ce.serial_ceva 
							and hrr.serial_hrr=ce.serial_hrr
							AND ce.serial_per ='.$par[0].'
					...
					/********va sacando el Total de todos los profesores filtrado por Periodo************/					
						...
						$promedioperiodo='select ce.serial_teva, sum( valor_rsp ) AS suma, count( DISTINCT ce.serial_ceva ) as num_eval
							from cabecera_evaluacion as ce, detalle_evaluacion as de, horario as hrr
							WHERE de.serial_ceva = ce.serial_ceva 
							and hrr.serial_hrr=ce.serial_hrr
							AND ce.serial_per ='.$par[0].'
