...
function consulta($seccion,$empleado){
		...
		$profesores='SELECT sum( valor_rsp )as suma, count(*) as numpreguntas, ce.serial_teva, count(serial_teva) as tipos, ce.serial_ceva, epl.serial_epl, epl.nombre_epl, mat.serial_mat, nombre_mat, count( DISTINCT arev.serial_arev ) AS numdeareas
					FROM cabecera_evaluacion ce, empleado AS epl, materia AS mat, horario AS hrr, detalle_evaluacion as de, pregunta_evaluacion AS prg, area_evaluacion AS arev
...
//////////////////////////////////////////////////echo "</p> SQL 2".$resultado."</p> SQL 2";
...
//formula para calcular la suma que da por cada area multiplicado para 100 todo eso dividido para la divicion de 100 para el numero de preguntas total multiplicado para el numero de preguntas por area
						...
						$total[1][$i] = $total[1][$i] + ((100*$retresultado->fields['sumaxarea'])/((100/$retprofesor->fields['numpreguntas'])*$retresultado->fields['numpreguntaxarea']));
									//echo "|";
				 ...
				 /*$datosprofesor='SELECT ce.serial_epl, ce.serial_mat, nombre_mat, concat_ws(" ",apellido_epl, nombre_epl) as docente, hrr.seccion_hrr, de.serial_prg, prg.serial_arev, de.serial_deva, de.serial_ceva, ce.serial_teva, arev.nombre_arev, fac.nombre_fac, per.nombre_per, are.nombre_are,hrr.seccion_hrr, are.serial_are, fac.serial_fac
FROM detalle_evaluacion AS de, cabecera_evaluacion ce, empleado as epl, materia as mat, horario as hrr, pregunta_evaluacion as prg, area_evaluacion as arev,facultad as fac, periodo as per,area as are
...
function consultaConsolidada($area,$facultad){
  ...
  global $DBConnection;
		...
		/********va sacando el Total de de caa uno de los profesores filtrado por area y materia************/					
				...
				if($retpromedio->RecordCount()>0){	
					...
					while(!$retpromedio->EOF ){						
						if($retpromedio->fields['serial_teva']==1){
							$porcentaje = $porcentaje +	(50 * ($retpromedio->fields['suma']/$retpromedio->fields['num_eval']))/100;													
						}
						if($retpromedio->fields['serial_teva']==2){
							$porcentaje = $porcentaje	+ (10 * ($retpromedio->fields['suma']/$retpromedio->fields['num_eval']))/100;							
						}
						if($retpromedio->fields['serial_teva']==3){
							$porcentaje = $porcentaje +	(20 * ($retpromedio->fields['suma']/$retpromedio->fields['num_eval']))/100;							            			}							
...
/********va sacando el Total de de caa uno de los profesores filtrado por facultad y materia************/					
					...
					while(!$retpromediofacultad->EOF ){						
						if($retpromediofacultad->fields['serial_teva']==1){
							$porcentajef = $porcentajef +	(50 * ($retpromediofacultad->fields['suma']/$retpromediofacultad->fields['num_eval']))/100;				
																
						}
						if($retpromediofacultad->fields['serial_teva']==2){
							$porcentajef = $porcentajef	+ (10 * ($retpromediofacultad->fields['suma']/$retpromediofacultad->fields['num_eval']))/100;
							}
						if($retpromediofacultad->fields['serial_teva']==3){
							$porcentajef = $porcentajef +	(20 * ($retpromediofacultad->fields['suma']/$retpromediofacultad->fields['num_eval']))/100;			
							}
					...
					/********va sacando el Total de de caa uno de los profesores filtrado por Periodo y materia************/					
					...
					while(!$retpromedioperiodo->EOF ){						
						if($retpromedioperiodo->fields['serial_teva']==1){
							$porcentajep = $porcentajep +	(50 * ($retpromedioperiodo->fields['suma']/$retpromedioperiodo->fields['num_eval']))/100;				
																
						}
						if($retpromedioperiodo->fields['serial_teva']==2){
							$porcentajep = $porcentajep	+ (10 * ($retpromedioperiodo->fields['suma']/$retpromedioperiodo->fields['num_eval']))/100;
							}
						if($retpromedioperiodo->fields['serial_teva']==3){
							$porcentajep = $porcentajep +	(20 * ($retpromedioperiodo->fields['suma']/$retpromedioperiodo->fields['num_eval']))/100;			
							}
