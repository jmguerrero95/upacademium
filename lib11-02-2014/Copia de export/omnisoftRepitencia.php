...
<?php
	...
	$sPer = "
			...
			FROM 
				periodo 
			WHERE 
			serial_per=".$serial_per_ini
	
	$rsPeriodo_ini=$dblink->Execute($sPer);
	...
	$repetidos = "
		SELECT  
			mmat.SERIAL_ALU,
			concat_ws(' ', alu.apellidopaterno_alu,
		...
		GROUP BY 
			alu.serial_alu, mat.serial_mat
		HAVING 
			count(*) > 1
		ORDER BY 
			mat.nombre_mat, matricula desc, nombre";
