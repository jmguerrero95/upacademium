...
$arrMainPrior = 
	...
	(
		...
		3	=>	"DIPLOMADO",
		4	=>	"TERCER NIVEL",
		5	=>	"IV DOCTOR EN JURISPRUDENCIA O FILOSOFIA",
...
$arrTercerNivel = 
	array(
		...
		2 => "III DOCTOR EN JURISPRUDENCIA O FILOSOFIA",
		3 => "TECNOLOGO",
		4 => "TECNICO SUPERIOR"			 
	);
...
if($serial_sec == "1" or $serial_sec == 'T'){
	$condPregTodas = "
	OR(
            periodo.serial_sec = 1 AND fechorario_hrr is null 
     )	
	";
}
...
WHERE
    ...
    AND( 
        (
            periodo.serial_sec = 2 
            AND hrr.serial_hrr IN( 
                        SELECT
                            serial_hrr 
                        FROM
                            horario 
                        WHERE
                            fechorario_hrr>='".$fecha_ini."' 
...
for($i=0;$i<$totAll;$i++){	
	...
	if($key == "" or $key == "TITULO TEMPORAL" or $key == "SIN TITULO"){
		...
		$horaAcadem =  round($arrMain[$i]['horadedicacion'],0);
