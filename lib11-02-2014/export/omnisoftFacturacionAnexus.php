...
case when sum(det.valor_aal)>0 then sum(det.valor_aal*det.cantidad_dfa+det.descuento_dfa) else
sum(det.valor_aal*det.cantidad_dfa-det.descuento_dfa) end basetarifacero,
...
<table width="100%" align="center">
  ...
  <tr>
    <td colspan="3" bgcolor="#FFFFFF"  style="font:Arial, Helvetica, sans-serif; font-style:inherit"  >&nbsp;</td>
  </tr>
  <?php
  ///////////////////////////////////////////////////// 
  for($n=0;$n<=1;$n++){
	...
	switch($n){
				case 0: 						
					  	$rsalumno=anexus($fecha_ini, $fecha_fin, $serial_suc," and cab.estado_caf <> 'ANULADO' ");
						break;
				case 1:
						echo ' <tr>  <td colspan="12" bgcolor="#FFFFFF" style="font:Arial, Helvetica, sans-serif; font-style:inherit" align="center" >FACTURAS ANULDAS</td>  </tr>';				
					  	$rsalumno=anexus($fecha_ini, $fecha_fin, $serial_suc," and cab.estado_caf = 'ANULADO' ");
						break;									
				}
    ...
    <td colspan="3" bgcolor="#FFFFFF"><table width="100%" border="1" cellpadding="0" cellspacing="0" vspace="0">
		...
		<? $i=1;
			...
			?>			
				...
				<td ><span class="style2"><? echo $rsalumno->fields['nombre'];?></span></td>
				<td ><span class="style2"><? echo $rsalumno->fields['identificacion'];?></span></td>
...
function anexus($fecha_ini, $fecha_fin, $serial_suc,$estado){
		require('../adodb/adodb.inc.php'); 
        require('../../config/config.inc.php');
	...
	$sql_alumno="select  concat_ws(' ',apellidopaterno_alu,apellidomaterno_alu,nombre1_alu,nombre2_alu) as nombre, codigoIdentificacion_alu as identificacion,
		cab.fecha_caf as fecha,
		month(cab.fecha_caf) as mes,
		year(cab.fecha_caf)as anio,
		cab.cedula_caf as descripcion,
		...
		sum(0) as base_imponible_ice,
		sum(0) as monto_ice
		from cabecerafactura cab, detallefactura det, alumno as alu
		where cab.serial_caf=det.serial_caf
		and cab.serial_alu=alu.serial_alu 
		and cab.fecha_caf>='$fecha_ini' and cab.fecha_caf<='$fecha_fin'
		and tipo_caf <> 'ND' 
		and cab.serial_suc = $serial_suc		
		".$estado."
