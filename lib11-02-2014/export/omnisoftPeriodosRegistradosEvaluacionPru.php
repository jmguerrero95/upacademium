...
";
//echo "<br>Main:<br>".$sqlMain."<br>";
//AND mmat.serial_alu = 3854				AND mmat.serial_alu = 5155
//AND mmat.serial_alu = 6673
$arrMain = $dblink->GetAll($sqlMain);
$totAll = count($arrMain);
//Parametrizacion
for($i=0;$i<$totAll;$i++){	
	$arrCT = getCreditosAprobadosALaFecha($arrMain[$i]['serial_alu'],$arrMain[$i]['serial_suc'],$arrMain[$i]['serial_sec'],$arrMain[$i]['fecfin_per'],$arrMain[$i]['serial_maa'],$dblink);
	if($arrCT){	
		//print "<pre>";print_r($arrCT);print "<pre>";
		$arrMain[$i]['creditosaprobadosalafecha'] = $arrCT;
	}

	$arrCNA = getCreditosNivelActual($arrMain[$i]['serial_alu'],$arrMain[$i]['serial_suc'],$arrMain[$i]['serial_sec'],$arrMain[$i]['fecini_per'],$arrMain[$i]['fecfin_per'],$arrMain[$i]['serial_maa'],$dblink);
	if($arrCNA){	
		//print "<pre>";print_r($arrCT);print "<pre>";
...
function getCreditosAprobadosALaFecha($serial_alu,$serial_suc,$serial_sec,$fecha_fin,$serial_maa,$dblink){
	$sqlGet = "
		SELECT
		    dma.serial_dma,
...
function getCreditosNivelActual($serial_alu,$serial_suc,$serial_sec,$fecha_ini,$fecha_fin,$serial_maa,$dblink){
	$sqlGet = "
		SELECT
