...
/*if($serial_sec==2)
		$porCarrera =" car.nombre_car like '".$serial_car."' AND ";
	else
		$porCarrera =" crp.nombre_crp like '".$serial_car."' AND ";
...
switch($nivelSeccion){
						...
						$periodoMalla="";
								...
								switch($bandera){
										case 'C': 
												...
												break;
										case 'N': 
													if($nivelEst=='TECNICO SUPERIOR')
														$porCarrera = "car.nombre_car like 'ASOCIADO%' AND ";
													else	
														$porCarrera = "car.nombre_car not like 'ASOCIADO%' AND ";
						...
						$periodoMalla=" ((fecini_per >='".$fecha_ini."' and fecini_per<='".$fecha_fin."') or (fecfin_per >='".$fecha_ini."' and fecfin_per<='".$fecha_fin."')) AND ";						
							...
							switch($bandera){
										case 'C': 
												...
												break;
										case 'N': 
													if($nivelEst=='DIPLOMADO')
														$porCarrera = "car.nombre_car like 'DIPLOMA%' AND ";
													else	
														$porCarrera = "car.nombre_car not like 'DIPLOMA%' AND ";
				...
				case 'T':
						...
						switch($bandera){
										case 'C': 
												$porCarrera =" crp.nombre_crp like '".$serial_car."' AND ";
												$porCarrera2 =" car.nombre_crp like '".$serial_car."' AND ";
												break;
										case 'N': 
...
switch($nivelSeccion){
				case 1: 						
						$sqlAlumnoMalla[0] = $sqlAlumnoMalla[1].$nivel.$sqlAlumnoMalla[2].$porCarrera.$seccionPregrado.$sqlAlumnoMalla[3].$seccionMatPregrado.$sqlAlumnoMalla[4].$sqlAlumnoMalla[5]; //pregrado
						break;
				case 2:
						$sqlAlumnoMalla[0] = $sqlAlumnoMalla[1].$nivel.$sqlAlumnoMalla[2].$porCarrera.$seccionPostgrado.$periodoMalla.$sqlAlumnoMalla[3].$seccionMatPostgrado.$sqlAlumnoMalla[4].$sqlAlumnoMalla[5]; //postgrado
						break;					
				case 'T':
						
						$sqlAlumnoMalla[0] = $sqlAlumnoMalla[1].$nivel.$sqlAlumnoMalla[2].$porCarrera.$seccionPregrado.$sqlAlumnoMalla[3].$seccionMatPregrado.$sqlAlumnoMalla[4]." UNION ".$sqlAlumnoMalla[1].$nivel2.$sqlAlumnoMalla[2].$porCarrera2.$seccionPostgrado.$periodoMalla.$sqlAlumnoMalla[3].$seccionMatPostgrado.$sqlAlumnoMalla[4].$sqlAlumnoMalla[5];
						break;
				}
...
<div id="itsthetable">
  ...
  <table summary="Profesores PEI">
	<caption>
	Estudiantes Matriculados en <?php echo $serial_car.$nivelEst." (".$numAlum."). Desde: ".$fecha_ini."  Hasta: ".$fecha_fin;?>
	</caption>
...
<div id="accordion">
 ...
 <?php for($y=1;$y<count($vecNivel);$y++){?>
	 <tr> 		
