...
for($i=0;$i<$totAll;$i++){	
	...
	$arr = getCreditoMain($serial_alu,$arrMain[$i]['serial_mat'],$dblink);
	if($arr){
...
for($i=0;$i<$totAll;$i++){		
	$totalCreditosRegistro = $totalCreditosRegistro + $arrMain[$i]['creditosmateria'];	
	$totalCreditosMalla = $totalCreditosMalla + $arrMain[$i]['creditomalla'];	
	$totalNotaNumericaPorCredito = $totalNotaNumericaPorCredito +($arrMain[$i]['creditosmateria']*$arrMain[$i]['notanumerica']);			
	switch($arrMain[$i]['estado_nal']){
		case 'APRUEBA':
			$totalNotaFinal = $totalNotaFinal + $arrMain[$i]['notatotal_nal'];
			$totalCreditosAprueba = $totalCreditosAprueba + $arrMain[$i]['creditosmateria'];	
			$totalNotasAprueba++;
			break;
		case 'REPRUEBA':
			$totalCreditosReprueba = $totalCreditosReprueba + $arrMain[$i]['creditosmateria'];	
			break;			
	}	
}
	...
	<table width="100%" border="1" cellpadding="0" cellspacing="0" vspace="0">
		...
		<?php  
		 ...
		 for($i=0;$i<$totAll;$i++){	
		  ...
		  <tr>
		     ...
		     <td  align="left" nowrap><? echo $arrMain[$i]['creditosmateria'];?> </td>		 
		...
		?>
	      <tr>
		    ...
		    <td  align="left" nowrap><? echo 0;?></td>
