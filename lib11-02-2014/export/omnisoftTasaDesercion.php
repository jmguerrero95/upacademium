...
/*$sql_alumnos="select max(periodo.fecini_per) ultimo_periodo,COUNT( numerocreditos_dmat+creditosequivalentes_dmat) creditos ,codigo_alu,materiamatriculada.serial_alu,concat_ws(' ', apellidopaterno_alu,apellidomaterno_alu,nombre1_alu,nombre2_alu) as nombre,
codigoIdentificacion_alu as identificacion, concat_ws(' / ',telefodomic_alu, celular_alu) as telefono,
 mailuniv_alu as correo, fecegresamiento_ama egreso,fectitulacion_ama titulo
,fecestado_hac retiro,concat_ws(' ',motivos_hac,descmotivos_hac) motivo
,round(DATEDIFF(now(),max(periodo.fecini_per))/365) as anios
from alumno,periodo, periodo as periodo1,materiamatriculada,detallemateriamatriculada,alumnomalla,malla
left join historicoacademico on historicoacademico.serial_alu=alumno.serial_alu
join notasalumnos on notasalumnos.serial_dmat=detallemateriamatriculada.serial_dmat and estado_nal='APRUEBA'
...
and alumnomalla.serial_alu=alumno.serial_alu  
and malla.serial_maa=alumnomalla.serial_maa

and fecegresamiento_ama='0000-00-00' 
...
group by serial_alu
UNION
select periodo.fecini_per AS ultimo_periodo,0 creditos,codigo_alu,alumno.serial_alu,concat_ws(' ', apellidopaterno_alu,apellidomaterno_alu,nombre1_alu,nombre2_alu) as nombre,
codigoIdentificacion_alu as identificacion, concat_ws(' / ',telefodomic_alu, celular_alu) as telefono,
 mailuniv_alu as correo, fecegresamiento_ama egreso,fectitulacion_ama titulo
,' ' retiro
,' ' motivo
,-99 as anios
from alumno, periodo ,alumnomalla,malla
where 
 alumno.serial_per=periodo.serial_per
and periodo.fecini_per >='".$fecha_ini."' and periodo.fecini_per<'".$fecha_fin."'
and alumnomalla.serial_alu=alumno.serial_alu  
and malla.serial_maa=alumnomalla.serial_maa
and malla.serial_maa_p=0
and fecegresamiento_ama='0000-00-00' 
...
$sql_alumnos="
SELECT 
alumno.serial_alu,concat_ws(' ', apellidopaterno_alu,apellidomaterno_alu,nombre1_alu,nombre2_alu) as nombre,
codigoIdentificacion_alu as identificacion, concat_ws(' / ',telefodomic_alu, celular_alu) as telefono,
 mailuniv_alu as correo,
sum(numerocreditos_dmat+creditosequivalentes_dmat) creditos,
fecestado_hac retiro,concat_ws(' ',motivos_hac,descmotivos_hac) motivo,
round(DATEDIFF(now(),max(periodo.fecini_per))/365) as anios,
max(periodo.fecini_per) ultimo_periodo,
totalcreditos_maa
...
WHERE 
notasalumnos.serial_dmat=detallemateriamatriculada.serial_dmat and estado_nal='APRUEBA'
and materia.serial_mat=detallemateriamatriculada.serial_mat 
and (fecegresamiento_ama='0000-00-00' OR fectitulacion_ama='0000-00-00')
and periodo.fecini_per >='".$fecha_ini."' and periodo.fecini_per<'".$fecha_fin."'
and alumno.serial_suc=".$serial_suc." and alumno.serial_sec=".$serial_sec."
and (intercambio_alu<>'INTERCAMBIO' and intercambio_alu<>'COMUNIDAD')
group by alumno.serial_alu
order by  nombre

";
//--materiamatriculada.serial_alu =2103
...
<table width="100%" align="center">
  ...
  <tr>
    <td colspan="3" bgcolor="#FFFFFF"><table width="100%" border="1" cellpadding="0" cellspacing="0" vspace="0">
		...
		<td width="1%" class="textovertical" bgcolor="#BEEEF8"><span class="style2">CREDITOS</span></td>
		<td width="1%" class="textovertical" bgcolor="#BEEEF8"><span class="style2">CREDITOS MALLA</span></td>
		</tr>
		...
		<? $i=1;
			...
			$AnosEgresados=array();
			while (!$rsalumnos->EOF) {
			   if ($rsalumnos->fields['creditos']<$rsalumnos->fields['totalcreditos_maa']){
			...
			?>
				...
				<td class="style2"><? echo $rsalumnos->fields['totalcreditos_maa'];?></td>
				</tr>
