...
//				$carreraMalla = $rsAlumnoMalla->fields['nombre_car'];			
			...
			$vecCreditos = funCreditos($rsAlumnoMalla->fields['serial_maa'], $rsAlumnoMalla->fields['serial_alu'],$rsAlumnoMalla->fields['serial_sec']);			
...
function funCreditos($malla, $alumno,$serial_sec){
require('../adodb/adodb.inc.php');
require('../../config/config.inc.php');
global $dblink;
		$creditos=0;	
...
$sqlGet = "
	...
	if($arr = $dblink->GetAll($sqlGet)){
		...
		for($i=0;$i<count($arr);$i++){			
			$creditos =$creditos+$arr[$i]['creditostomados'];
		}		
		//$creditos = $ret +$ncredOther;
		//return $ret;
	}
