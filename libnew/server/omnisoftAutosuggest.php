  ...
  global  $MAX_AUTOSUGGEST_ITEMS;
	...
	$len = strlen($input);
        ...
        $fieldinfo = strtolower( $_GET['fieldinfo'] );
           ...
           if ($len)
           $query="select ".$fieldid.",".$fieldname.",".$fieldinfo." from ".$table." where (".$fieldname." like '".$input."%') or (".$fieldinfo." like '".$input."%') order by ".$fieldname ;
           else
           $query="select ".$fieldid.",".$fieldname.",".$fieldinfo." from ".$table ." order by ". $fieldname;
