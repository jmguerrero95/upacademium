...
if	($par[1]=='TODOS'){	
	$todos = "select SERIAL_ALU, ce.SERIAL_EPL, SERIAL_MAT,serial_eva
			from cabecera_evaluacion as ce, empleado as epl 
			where ce.SERIAL_EPL = epl.SERIAL_EPL
			and SERIAL_TEVA = 1 
			and ce.SERIAL_PER = ".$par[0]." 
			group by SERIAL_ALU, ce.SERIAL_EPL, SERIAL_MAT
...
$todos = "select SERIAL_EPL, SERIAL_MAT, serial_eva 
			from cabecera_evaluacion 
			where SERIAL_TEVA = 1 
			and cabecera_evaluacion.SERIAL_PER = ".$par[0]."  
		...
		$bandera=0;
			while(!$rettodos->EOF ){
					...
					if	($par[1]=='TODOS'){	
						...
						$alumno=$rettodos->fields['SERIAL_ALU'];						
					}
					else{					
						$empleado=0;
						$materia=0;
...
$result = consulta(1,$empleado,$materia,$alumno) 
?>
...
<table align="center" width="80%">
  ...
  <tr>
    ...
    <td class="style2"><? echo  $result->fields['alumno'];  ?>  </td>
  </tr>   
</table>
...
<tr>
	<td class="style3">Evaluación:</td>
	<td class="style3" align="right"> Respuestas <? echo $result->fields['tipo_teva']; ?>:</td>	
</tr>  
</table>
  		...
  		<table width="100%" border="1" cellspacing="0" >
			...
			<? 
			$respuestas1= "select serial_rsp,nombre_rsp  from respuesta WHERE serial_eva=".$rettodos->fields['serial_eva']." order by serial_rsp";
			$rs_respuestas = $dblink->Execute($respuestas1);
			//echo $respuestas1; 
			if($rs_respuestas->RecordCount()>0){?>			
			<tr><td>AREA</td><td>PREGUNTAS</td>			
			<?
			
			while(!$rs_respuestas->EOF ){
			?>
			<td><? echo $rs_respuestas->fields['nombre_rsp']; ?></td>
			<? $rs_respuestas->MoveNext(); }?>
			<td>N/A</td>
			<? }?>
			</tr>
			<? detalleEvaluacion($result->fields['serial_ceva'],$rs_respuestas); ?>
</table>	
...
<table align="center" width="80%" border="1" cellspacing="0" cellpadding="5" bordercolordark="#000000">
			...
			<? consulta(2,$empleado,$materia,$alumno) ?>
</table>	
		</td>
	</tr>
</table>
...
<table align="center" width="80%">
	<tr>
		<td class="style3">CUARTA SECCIÓN:</td>
	</tr>  
</table>
...
function consulta($seccion,$empleado,$materia,$alumno){
		global $DBConnection;
      	$dblink = NewADOConnection($DBConnection);
 		$url=$_GET['query1'];		
		$par = explode("|",$url);			
//Pone en cero los parametros de periodo empleado materia y tipo de evaluacion si esque se pasa un valor vacio PARA QUE NO DE ERROR LA CONSULTA
		...
		$profesores='SELECT sum( de.valor_rsp )as suma, count(*) as numpreguntas, ce.serial_teva, count(serial_teva) as tipos, ce.serial_ceva, epl.serial_epl, epl.nombre_epl, mat.serial_mat, nombre_mat, count( DISTINCT arev.serial_arev ) AS numdeareas,max(rsp.valor_rsp) as maximovalor
					FROM cabecera_evaluacion ce, empleado AS epl, materia AS mat, horario AS hrr, detalle_evaluacion as de, pregunta_evaluacion AS prg, area_evaluacion AS arev, detallemateriamatriculada as dm, materiamatriculada as mm,respuesta as rsp
					...
					AND ce.serial_dmat = dm.serial_dmat
					AND dm.serial_mmat = mm.serial_mmat
					and de.serial_rsp = rsp.serial_rsp
					and de.NA_RSP <> "SI"
					AND ce.serial_per ='.$par[0].'
					AND ce.serial_epl ='.$par[1].'
					AND ce.serial_mat ='.$par[2].'						
					AND mm.serial_alu ='.$par[3].'			
...
FROM detalle_evaluacion AS de, cabecera_evaluacion ce, empleado as epl, materia as mat, horario as hrr, pregunta_evaluacion as prg, area_evaluacion as arev,facultad as fac, periodo as per,area as are, tipo_evaluadores as teva,  detallemateriamatriculada as dm, materiamatriculada as mm,alumno as alu  
					 ...
					 AND ce.serial_dmat = dm.serial_dmat
					 AND dm.serial_mmat = mm.serial_mmat
					 and ce.serial_alu=alu.serial_alu
					 and de.NA_RSP <> "SI"
					 AND ce.serial_per ='.$par[0].'
					 AND ce.serial_epl ='.$par[1].'
					 AND ce.serial_mat ='.$par[2].'
...
function detalleEvaluacion($ceva,$resp){
//$ceva = 204;
...
$dblink = NewADOConnection($DBConnection);
	...
	if ($ceva!=""){
			$detalle='select prg.SERIAL_PRG,prg.DETALLE_PRG , de.serial_rsp,arev.SERIAL_AREV, NA_RSP, arev.nombre_arev
					FROM detalle_evaluacion AS de, 
					pregunta_evaluacion AS prg, 
					area_evaluacion AS arev 
					WHERE serial_ceva ='.$ceva.'
					AND de.serial_prg = prg.serial_prg 
					AND prg.serial_arev = arev.serial_arev
					order by arev.nombre_arev';
					...
					//echo "</p>".$detalle."</p>";
					//and NA_RSP <> "SI" 			
					$retdetalle=$dblink->Execute($detalle);
					if($retdetalle->RecordCount()>0){
					while(!$retdetalle->EOF ){
						echo "<tr>";				
						echo "<td class='style5'>".$retdetalle->fields['nombre_arev']."</td>";
						echo "<td class='style5'>".$retdetalle->fields['DETALLE_PRG']."</td>";
						$resp->MoveFirst();
						while(!$resp->EOF){
							if ($retdetalle->fields['serial_rsp'] == $resp->fields['serial_rsp']){
								//Cuando la respuesta es NA
								if($retdetalle->fields['NA_RSP']=='SI') echo "<td width='2%' align='center' class='style2'>&nbsp;</td>";
								echo "<td width='2%' align='center' class='style2'>"." * "."</td>";							
							}else	
								echo "<td width='2%' align='center' class='style2'>&nbsp;</td>";
