...
if	($par[1]=='TODOS'){	
	$todos = "select ce.SERIAL_CEVA, ce.SERIAL_EPL  
			...
			$todos = "select ce.SERIAL_CEVA, ce.SERIAL_EPL, SERIAL_TEVA
						from cabecera_evaluacion as ce, empleado AS epl, horario as hrr 
					WHERE
						ce.SERIAL_EPL=epl.SERIAL_EPL AND
						ce.SERIAL_HRR=hrr.SERIAL_HRR AND
						hrr.estado_hrr LIKE 'ACTIVO' AND
						hrr.SERIAL_PER = ".$par[0]."
...
/////////////////////////////////////////////////echo "</p> SQL 1 ".$profesores;
				...
				if($retprofesor->RecordCount()>0){
				 while(!$retprofesor->EOF ){					
						...
						$resultado= 'SELECT count( * ) as numpreguntaxarea,
						             de.serial_rsp, sum( valor_rsp ) as sumaxarea,
									 arev.nombre_arev,	arev.serial_arev 
...
//formula para calcular la suma que da por cada area multiplicado para 100 todo eso dividido para la divicion de 100 para el numero de preguntas total multiplicado para el numero de preguntas por area
							...
							echo "</p> SUMA X AREA ".$retresultado->fields['sumaxarea']."</p>";
							echo "</p> MAX TOT ".$retprofesor->fields['maximovalor']."</p>";
							echo "</p> PREG X AREA ".$retresultado->fields['numpreguntaxarea']."</p>";*/
								  ...
								  //saca el total de filas en el vector
										...
										for ($i=1;$i<=$vec;$i++){
												if($total[2][$i]==$retcalculos->fields['serial_arev']){													
													echo "<td class='style2' align='center'>".$tot = number_format((($total[1][$i]/$cont)*5)/100,2)."CONT: ".$cont."</td>";													
												$tot1 =$tot1+$tot;
