...
carrera.nombre_car as carrera, alumno.fechaInscripcion_alu as inscripcion
, max(periodo.fecini_per) as ultimoregistro, sum(numerocreditos_dmat+creditosequivalentes_dmat) as creditos
, totalcreditos_maa as totalcreditos, round(DATEDIFF(now(),max(periodo.fecini_per))/365) as anios, 
codigoIdentificacion_alu as identificacion, concat_ws(' / ',telefodomic_alu, celular_alu) as telefono,
 mailuniv_alu as correo, fecestado_hac as retiro, concat_ws(' ',motivos_hac,descmotivos_hac) as motivo
...
group by alumno.serial_alu, malla.serial_maa";
