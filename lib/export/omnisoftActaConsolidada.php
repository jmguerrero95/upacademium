...
WHERE
	...
	AND notasalumnos.estado_nal like 'APRUEBA'
...
//Nota de tesis o trabajo de titulacion
if($serial_sec==1)
	$tesis=53;
if($serial_sec==2)
	$tesis=62;
...
WHERE
	dad.serial_dad=rgra.serial_dad 
	AND serial_ama in(select serial_ama from alumnomalla where serial_alu=".$serial_alu.") and
	dad.serial_dad=".$tesis;

$rsTesis=$dblink->Execute($sqlNotaTesis);

//echo $sqlNotaTesis;

//pasantia
$sqlPasantias="SELECT
	horaspasantias,area_rgra 
FROM
	documentosadmision  AS dad,
	requisitosgraduacion AS rgra 
WHERE
	dad.serial_dad=rgra.serial_dad AND serial_ama in(select serial_ama from alumnomalla where serial_alu=".$serial_alu.") and
	dad.serial_dad=17";
	
$rsPsanatias=$dblink->Execute($sqlPasantias);	
//echo $sqlPasantias;
/////comuniudad
...
WHERE
	dad.serial_dad=rgra.serial_dad AND serial_ama in(select serial_ama from alumnomalla where serial_alu=".$serial_alu.") and
	dad.serial_dad=20";
$rsComunidad=$dblink->Execute($sqlComunidad);	
    ...
    <td width="71%" bgcolor="#FFFFFF"><table width="100%" border="0">
      <tr>
        <td class='style1' align="center">
          <p  >ACTA CONSOLIDADA </p></td>
      </tr>
	  <tr bgcolor="#FFFFFF">
        <td class='style1' align="center"><span class=""><? echo $arrAlu[0]['nombre_crp'];?> </span></td>
      </tr>
      <tr>
  ...
  <tr>
    <td colspan="2" bgcolor="#FFFFFF"><table width="100%" border="1">
      ...
      <tr>
        ...
        <td align="right" class='style2'>Ver Materias  Fuera de Malla:</td>
	...
	<table width="100%" border="1" cellpadding="0" cellspacing="0" vspace="0">
		...
		<?php  
		 ...
		 for($i=0;$i<$totAll;$i++){	
			...
			?>
		   <tr>
		     <td  align="left" class="style3"><? echo $i + 1; ?></td>
       ...
       <!-- <tr>
		...
		<tr>
		  ...
		  <tr>
          <td colspan="7" class='style2'><div align="right">Calificacion Trabajo de Titulación/Trabajo de Conclusión de Carrera: </div></td>
		  <td  class='style3'><?php echo $rsTesis->fields['nota_rgra']; ?> </td>
		  <td >&nbsp;</td>
		  </tr>
		  <tr>
          <td colspan="7" class='style2'><div align="right">Área de Pasantías: </div></td>
		  <td  class='style3'><?php echo $rsPsanatias->fields['area_rgra']; ?> </td>
		  <td >&nbsp;</td>
		  </tr>		   
		   <tr>
          <td colspan="7" class='style2'><div align="right">Horas de Pasantías: </div></td>
		  <td  class='style3'><?php  echo $rsPsanatias->fields['horaspasantias']; ?> </td>
		  <td >&nbsp;</td>
		  </tr>
		   <tr>
          <td colspan="7" class='style2'><div align="right">Horas de Servicio a la Comunidad: </div></td>
		  <td  class='style3'><?php   echo $rsComunidad->fields['horaspasantias']; ?> </td>
		  <td >&nbsp;</td>
		  </tr>
		   <!-- <tr>
		  ...
		  <td >&nbsp;</td>
		   ...
		   <tr>
		     <td colspan="7" class='style2'><div align="right">Total CréditosS Tomados (Aprobados/Homologados/Revalidados) </div></td>
