...
CASE 
	...
	END as EstadoEstudiante,		 
		 ...
		 alu.telefodomic_alu as telefono_domicilio,
		 alu.telefotrabajo_alu as telefono_trabajo,
		 celular_alu as celular,
	CASE 
  ...
  <thead>
    <tr>
	 <th scope="col"><span class="style1">SERIAL(ocultar)</span></th>
      ...
      <th scope="col">numeros_creditos <?php echo " ".$fecha_fin;?></th>
	  ...
	  <th scope="col">T.Domicilio</th>
	  <th scope="col">T.Trabajo</th>
...
//				$carreraMalla = $rsAlumnoMalla->fields['nombre_car'];			
			...
			$fechaRegistro=$vecCreditos[4];
			
			
			$vecDiscapacidad=fundiscapacidad($rsAlumnoMalla->fields['serial_alu']);
			$discapacidad=$vecDiscapacidad[0];
...
/*		if(echo $rsAlumnoMalla->fields['descripcion_sex']==''){	
	...
	?>	
	 ...
	 <?php $j=$i;?>
		...
		<td><?php echo $pasantia[0];?></td>
		<td><?php echo $pasantia[1];?></td>
		<td><?php echo $pasantia[2];?></td>
		...
		<td><?php echo $discapacidad[1];?></td>
		<td><?php echo $discapacidad[2];?></td>
		<td><?php echo $discapacidad[3];?></td>
		<td><?php echo $discapacidad[4];?></td>
	...
	<td><?php echo $fechaRegistro; ?></td>
		...
		<td><?php echo $rsAlumnoMalla->fields['telefono_domicilio'];?></td>
		<td><?php echo $rsAlumnoMalla->fields['telefono_trabajo'];?></td>
		<td><?php echo $rsAlumnoMalla->fields['celular'];?></td>
...
function fundiscapacidad($alumno){
...
global $dblink;
			...
			/*******PASANTIAS***********/
		/*	$sqlPasantia="select nombre_dad,reg.entregado_rgra, reg.horaspasantias,reg.institucionpasantias
			...
			if($rsPasantia->recordCount()>0){
				$pasantia[0]=$rsPasantia->fields['entregado_rgra'];
				$pasantia[1]=$rsPasantia->fields['horaspasantias'];
				$pasantia[2]=$rsPasantia->fields['institucionpasantias'];
				}
			else{
				$pasantia[0]='NO';
				$pasantia[1]='0';
				$pasantia[2]='NO APLICA';
				}
				
			*/	

			/*******DISCAPACIDAD***********/
			$sqldiscapacidad="select serial_alu, discapacidad_alu, tipodiscapacidad_alu , carnet_conadis_alu,numero_carnet_alu,porcentaje_discapacidad from alumno where serial_alu = ".$alumno." and discapacidad_alu ='SI'";
							
						
			//echo "</p>".$sqlconvalidacion;				
			$rsDiscapacidad = $dblink->Execute($sqldiscapacidad);					
			
			if($rsDiscapacidad->recordCount()>0){
				$discapacidad[0]=$rsDiscapacidad->fields['discapacidad_alu'];	
				$discapacidad[1]=$rsDiscapacidad->fields['tipodiscapacidad_alu'];	
				$discapacidad[2]=$rsDiscapacidad->fields['carnet_conadis_alu'];	
				$discapacidad[3]=$rsDiscapacidad->fields['numero_carnet_alu'];	
				$discapacidad[4]=$rsDiscapacidad->fields['porcentaje_discapacidad'];	
							
				}
			else{
				$discapacidad[0]='NO APLICA';
				$discapacidad[1]='NO APLICA';
				$discapacidad[2]='NO APLICA';
				$discapacidad[3]='NO APLICA';
				$discapacidad[4]='NO APLICA';				
				}	
				
				
		
	$vector = array($discapacidad);		
	//echo $vector[0];
	return $vector; 
...
global $dblink;
		...
		$sqlMatricula="select fecini_per,materiamatriculada.SERIAL_MMAT, max(FECHAMATRICULA_MMAT) as fechaRegistro from materiamatriculada, detallemateriamatriculada, alumnomalla,malla,periodo where 
		...
		//echo "</p> MATRICULA 1 ".$sqlMatricula;
		 ...
		 if($rsMatricula->recordCount()>0) {
		 	 $matricula=$rsMatricula->fields['SERIAL_MMAT'];
			 $fechaRegistro=$rsMatricula->fields['fechaRegistro'];
			 }
	...
	$vector = array($creditos, $matricula, $pasantia,$convalidacion,$fechaRegistro);		
	//echo $vector[0];
	return $vector; 
}
...
global $dblink;
		...
		$sqlMatricula="select fecini_per,SERIAL_MMAT, max(FECHAMATRICULA_MMAT) as fechaRegistro from materiamatriculada,alumnomalla,malla,periodo where 
						materiamatriculada.SERIAL_ALU=alumnomalla.serial_alu 
...
//		echo "</p> Creditos FINAL".$creditos;	
	...
	$vector = array($creditos, $matricula, $pasantia,$fechaRegistro);		
	//echo $vector[0];
	return $vector; 
}
