	 ...
	 while (!$rsArea->EOF){	
		  ...
		  while (!$rsMaterias->EOF){
			if($rsNivel->fields['serial_niv']==$rsMaterias->fields['serial_niv'] && $rsArea->fields['serial_are']==$rsMaterias->fields['serial_are'] ){
			 	if($rsMaterias->fields['codigo_mat']<>""){
					$arrMat[$cont] = $rsMaterias->fields['codigo_mat']."-".$rsMaterias->fields['nombre_mat']."(".$rsMaterias->fields['numerocreditos_dma'].")<br>"; 
					echo "<br>MATERIA: ".$rsMaterias->fields['nombre_mat']."CONTADOR: ".$cont."<br>";
					$cont++;					
		 ...
		 //if($totMat>0){
		 	 $totMat = count($arrMat); 
			 echo "entro primera vez<br>";
			 echo "<span class='style2'><td align='center' nowrap>";
			 echo "";
			 for($i=0;$i<$totMat;$i++){
			 	echo $totMat[$i];
			 }
