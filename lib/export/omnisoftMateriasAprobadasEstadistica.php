...
<body>
 ...
 <table width="100%" >
	<caption>
	Resumido de <?php echo " (".$numAlum.") "; ?> Estudiantes Matriculados <?php echo " Desde: ".$fecha_ini."  Hasta: ".$fecha_fin;?>
	</caption>  
</table>
...
//			$creditos=$vecCreditos[0];
	...
	<p>
	  <?php		
			for($z=1;$z<=$count3;$z++){
					...
					$estadisticasNoTomadas[2][$q]=$materiasNoTomadas[2][$z];						
			}						
			...
			$vecMateriasEst = funEstadisticas($estadisticasNoTomadas,$q);
			
			if((count($vecMateriasEst,0))>0)	
				$count4 = (count($vecMateriasEst,1)/count($vecMateriasEst,0))-1;	
	...
	<table border="1px" width="100%" >
						...
						<tr>		
							...
							<td  scope="col" align="center"> <strong>Materia</strong></td> 
							<td  scope="col" align="center"><strong> No Estudiantes</strong></td>
						</tr>
						...
						?>	
						<tr>								
							<td> <?php echo " ".$p.".- ".$vecMateriasEst[1][$p];?> </td> 
							<td> <a href="omnisoftMatNoaprobadasEst.php?fecha_ini=<? echo $fecha_ini; ?>&serial_suc=<? echo $serial_suc; ?>&serial_sec=<? echo $serial_sec; ?>&fecha_fin=<? echo $fecha_fin; ?>&serial_mat=<? echo $vecMateriasEst[3][$p]; ?>" target="_blank"><?php echo $vecMateriasEst[2][$p];?> </a></td>
						</tr>
						<?php
								$q++;
								$estadisticasNoTomadas[1][$q]=$materiasNoTomadas[1][$z];						
...
//		echo "</p>".$r." -> ".$estadisticasNoTomadas[1][$r];
		...
		$temporal = "INSERT INTO temporalEstadistica
			(materia, nombre,serial) values(".$r.",'".$estadisticasNoTomadas[1][$r]."',".$estadisticasNoTomadas[2][$r].")";
		$dblink->Execute($temporal);			
		
	}
	
	$sqlMateriasFaltantes="SELECT
							count(materia) as cuantos,nombre,serial
						FROM
							temporalEstadistica
						group by nombre";
			$s=0;									
						...
						if($rssqlMateriasFaltantes->recordCount()>0){												
							while(!$rssqlMateriasFaltantes->EOF){
								...
								$materiasFaltante[3][$s]=$rssqlMateriasFaltantes->fields['serial'];								
						...
						if($rsmateriasXalumno->recordCount()>0){												
							...
							if($rsmateriasHomologadas->recordCount()>0){						
								while(!$rsmateriasHomologadas->EOF){
									...
									$materiasNoTomadas[2][$n]=$rsMateriaXmalla->fields['serial_mat'];
									//Materia no tomadas
