...
SELECT
    ...
    AS identificacion,
    DATEDIFF( DATE_FORMAT( fectitulacion_ama, '%Y-%m-%d' ) ,DATE_FORMAT( fecnac_alu, '%Y-%m-%d' ) ) /365 AS edad,
	fecnac_alu,
	tiempotardoprimertrabajodgraduado_seg,
	tiempotardoprimeremprendgraduado_seg,
    codigo_alu,
    ''                                                                             
    AS nivel,
    ''                                                                             
    ...
    mail_alu,
    nombre_fac,
	relacionDependencia_seg,
    fectitulacion_ama                                                              
    AS ftitulacion,
    fecegresamiento_ama                                                            
    AS fegresamiento,
    carreraprincipal.nombre_crp                                                    
    AS carrera,	
    nombre_crp,
	nombre_suc,
    nombre_car,
    alu.serial_sec,
    CASE 
        WHEN  observacionesGenerales_seg = ''
        THEN 'MANUAL/ERRONEA' 
        ELSE 'MIGRADO AUTOMATICO'
    END                                                                                                 
    AS observacionesGenerales_seg,
	fechaencuesta_seg 
FROM
    alumno AS alu,
    sexo   AS sex,
	sucursal AS suc,
	seguimientograduado AS sal 
        LEFT JOIN alumnomalla 
        ON alumnomalla.serial_alu=alu.serial_alu 
        LEFT JOIN malla 
        ON malla.serial_maa=alumnomalla.serial_maa 
        AND serial_maa_p=0 
        LEFT JOIN carrera 
...
WHERE
    alu.serial_sex = sex.serial_sex ".$seccion." ".$sucursal." 
	AND suc.serial_suc = alu.serial_suc
    AND sal.serial_alu = alu.serial_alu 
	AND (intercambio_alu<>'VIENE INTERCAMBIO' 
    AND intercambio_alu<>'COMUNIDAD') 
	AND fechaencuesta_seg >= '".$fecha_ini."' 
    AND fechaencuesta_seg <='".$fecha_fin."' 
...
<table width="90%" align="center">
  <tr bgcolor="#FFFFFF">
    ...
    <th colspan="2"><strong>LISTA DE ALUMNOS ACTUALIZADOS EN SEGUIMIENTO A ALUMNOS GRADUADOS </strong></th>
  </tr>
  <tr >
  ...
  <tr>
	...
	<table width="100%" border="1" cellpadding="0" cellspacing="0" vspace="0">
		...
		<tr>
		  ...
		  <th width="7%">FECHA ENCUESTA </th>
		  <th width="7%">ORIGEN ENCUESTA </th>
		...
		<tr> 
				...
				<td ><?php echo $arrMain[$i]['fechaencuesta_seg'];?></td>
				<td ><?php echo $arrMain[$i]['observacionesGenerales_seg'];?></td>
				...
				<td ><?php echo $arrMain[$i]['edad'];?></td>
				<td ><?php echo $arrMain[$i]['tiempotardoprimertrabajodgraduado_seg'];?></td>
				<td ><?php echo $arrMain[$i]['tiempotardoprimeremprendgraduado_seg'];?></td>
				...
				<td ><?php echo $arrMain[$i]['relacionDependencia_seg'];?></td>
				...
				<td ><span><?php echo $arrMain[$i]['carrera'];?></span> </td>
