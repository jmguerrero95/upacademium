...
for($i=0;$i<$totAll;$i++){	
	...
	//getValorCreditos
	$arrVC = getValorCreditos($arrMain[$i]['serial_alu'],$dblink);
	
	if($arrVC){	
		//print "<pre>";print_r($arrCT);print "<pre>";
		$arrMain[$i]['pagar'] = $arrVC;
	}
      ...
      <table width="100%" border="1" cellpadding="0" cellspacing="0" vspace="0">
        <tr>
          <th>Nº.</th>
			...
			<th># Créditos que le Falta  </th>
			<th># Valor x Cr&eacute;dito </th>
			<th># Valor de Créditos Faltantes  </th>
            <!--<th>Nivel Actual </th>-->
            <th>Sucursal </th>
		    <th>Malla</th>
		    <th>Carrera</th>
		 ...
		 for($i=0;$i<$totAll;$i++){	
			...
			$diferencia=$arrMain[$i]['creditosmalla']-$arrMain[$i]['creditostomados'];
			 if($diferencia<=0){
			  ...
			  $aPagar= $diferencia * $arrMain[$i]['pagar'];
        ...
        <tr>
          <td  align="left" nowrap><? echo $i + 1; ?></td>
			   ...
			   <td  align="left" nowrap><? echo $arrMain[$i]['descripcion_sex'];?></td>
               <td  align="left" nowrap><? echo $arrMain[$i]['creditosmalla'];?></td>
               <td  align="left" nowrap><? echo $arrMain[$i]['creditostomados'];?></td>
			   
			   <td  align="left" nowrap><? echo $falta;?></td>
			   <td  align="left" nowrap><? echo number_format($arrMain[$i]['pagar'],2);?></td>
			   <td  align="left" nowrap><? echo number_format($aPagar,2);?></td>
        ...
        <?php 
         ...
         <tr>
          <th colspan="11" ><div align="right">TOTAL MATRICULADOS : </div></th>
...
function getCreditosTomados($serial_alu,$serial_suc,$serial_sec,$fecha_fin,$dblink){
	$sqlGet = "
		...
		WHERE
			...
			group by dma.serial_mat
	";
	//if ($serial_alu==2330)echo "<br>".$sqlGet."<br>";
...
function getValorCreditos($serial_alu,$dblink){
	$sqlGet = "
		SELECT
	 distinct(alu.serial_alu),
	dar.valor_dar,
	dde.porcentaje_dde * 100  as porcentaje,
	(valor_dar - (valor_dar * if(porcentaje_dde IS NULL,0,porcentaje_dde))) AS pagar
FROM
	aranceles     AS ara,
	detallearancel AS dar,
	alumno         AS alu 
		LEFT JOIN detalle_descuento AS dde 
		ON alu.serial_caa = dde.serial_caa AND
		dde.serial_dar = dar.serial_dar 
WHERE
	alu.serial_per = ara.serial_per AND
	ara.serial_ara = dar.serial_ara AND
	ara.portomar_ara <> 'SI' AND
    ara.credito_ara='SI' AND
	ara.estado_ara='ACTIVO' AND 
	alu.serial_alu = ".$serial_alu;
	...
	if($arr = $dblink->GetAll($sqlGet)){
		 count($arrTrimestresDef);
		for($i=0;$i<count($arr);$i++){			
			//$ret =$ret+$arr[$i]['pagar']." ---------- ".$arr[$i]['valor_dar']."---------".$arr[$i]['porcentaje'];
			$ret =$ret+$arr[$i]['pagar'];
		}
		
//		echo "</p> VALORRRRRR: ".$ret;
		//$ret = $arr[0]['creditostomados'];
		return $ret;
	}else{
		return false;
