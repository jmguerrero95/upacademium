...
$sqlFacturacion = "select serial_caf,concat_ws('-',serie_caf,numero_caf) as factura,codigo_alu,cedula_caf,cliente_caf,estado_caf,total_caf,fecha_caf,tipo_caf,
					CASE 
						WHEN cabecerafactura.serial_com < 1
							THEN 'ABIERTO' 
						ELSE 'CERRADO' 
						END AS estado 					
					from cabecerafactura
					left join alumno on alumno.serial_alu=cabecerafactura.serial_alu
					 where fecha_caf>='".$fecha_ini."' and fecha_caf<='".$fecha_fin."'  and cabecerafactura.serial_suc=".$_COOKIE['serial_suc'];

$result_facturacion=$dblink->Execute($sqlFacturacion);

//echo "</p>	---->".$sqlFacturacion;

//and tipo_caf='FAC'

$sqlCobros="select numero_cre,NOMBRE_FORC,concat_ws('-',serie_caf,numero_caf) numfac,valor_dre,numeroDocumento_dre,fecha_dre,nombre_ban, plazo_dre,referencia_dre,lote_dre,tarjetahabiente_dre,fecha_cre,estado_caf,
			CASE 
			WHEN cabecerarecibo.serial_com < 0 OR
				cabecerarecibo.serial_com > 0 
				THEN 'CERRADO' 
				ELSE 'ABIERTO' 
			END AS estado   
			...
			and fecha_cre>='".$fecha_ini."' and fecha_cre<='".$fecha_fin."' and tipo_caf='FAC' and estado_caf<>'ANULADO' and cabecerafactura.serial_suc=".$_COOKIE['serial_suc']."
			order by nombre_forc,numfac";

$result_cobros = $dblink->Execute($sqlCobros);
  ...
  <tr>
    <td height="67" colspan="2" valign="top" bgcolor="#FFFFFF"><table width="100%" border="1" cellpadding="0" cellspacing="0" vspace="0">
      <tr>
        ...
        <th width="12%"><span class="style5">Tipo  </span></th>
		<th width="12%"><span class="style5">N&ordm;.  </span></th>
        ...
        <th width="4%" ><span class="style5">Valor</span></th>
		 <th width="4%" ><span class="style5">Estado Cierre</span></th>
      ...
      <tr>
        ...
        <td class="style2"><? echo $result_facturacion->fields['tipo_caf'];?></td>
	    ...
	    <td class="style2"><? echo $result_facturacion->fields['estado'];?> </td>
		</tr>
        <?
 
			if($result_facturacion->fields['estado_caf']!='ANULADO')
			 switch ($result_facturacion->fields['tipo_caf']) {
				case 'FAC':
					$total_fac=$total_fac+$result_facturacion->fields['total_caf'];	
					break;
				case 'NC':
					$total_nc=$total_nc+$result_facturacion->fields['total_caf'];	
					break;
				case 'ND':
					$total_nd=$total_nd+$result_facturacion->fields['total_caf'];	
					break;
			}
    ...
    <td valign="top" bgcolor="#FFFFFF"><table width="100%" border="1" cellpadding="0" cellspacing="0" vspace="0">
      <tr>
		...
		<th width="4%" class="style5" >Estado Cierre</th>
		</tr>
        <? //$i=1;
		 $m = 0;
		 $f = 0;
      ...
      <tr>
		...
		<td class="style2"><? echo $result_cobros->fields['estado'];?></td>		
		</tr>
        <?
    ...
    <td colspan="3" bgcolor="#FFFFFF"><table width="100%" border="0">
        <tr>
          ...
          <td width="51%" align="center"><div align="left">
			...
			<p>TOTAL Notas Crédito : <? echo " ".$total_nc;?></p>
			<p>TOTAL Notas Débito : <? echo " ".$total_nd;?></p>
