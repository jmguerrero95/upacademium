...
CASE 
	...
	CASE 
		...
		THEN 'YANUNCAY' 
	END                                                 AS parroquia ,
	estado_hac,his.fecestado_hac,per.nombre_per,";
	...
	alumno      AS alu 
		...
		ON ciu.serial_ciu=alu.serial_ciu 
			...
			ON alu.serial_prv = prv.SERIAL_PRV 
				...
				left join historicoacademico as his
                  on alu.serial_alu=his.serial_alu  
WHERE
	...
	per.fecini_per >= '2010-10-12' AND
	(intercambio_alu<>'VIENE INTERCAMBIO' AND
	intercambio_alu<>'COMUNIDAD') AND	
	";
  ...
  <thead>
    <tr>
	  ...
	  <th scope="col">Periodo de Ingreso</th>
	   <th scope="col">Periodo PreUni</th>	  
	  <th scope="col">Fecha PreUni</th>
	...
	<tfoot>
		<tr>
			<th scope="row">Total</th>
				<td colspan="33"><?php echo $numAlum." Alumnos";?></td>
		</tr>
	</tfoot>
	<tbody>
    <?php 
...
$niveles= getTrimestre($rsAlumnoMalla->fields['serial_alu'],$rsAlumnoMalla->fields['fecini_per'],$rsAlumnoMalla->fields['fecini_per'],$rsAlumnoMalla->fields['serial_sec'],$dblink);

$preuniversitario = funPreuniversitario($rsAlumnoMalla->fields['serial_alu'],$dblink);
		$periodoPre = $preuniversitario[0];
		$fechaPre = $preuniversitario[1];
...
/*		if(echo $rsAlumnoMalla->fields['descripcion_sex']==''){	
	...
	?>	
	 ...
	 <?php $j=$i;?>
		...
		<td><?php echo $rsAlumnoMalla->fields['totalcreditos_maa']; ?></td>
		<td><?php echo $rsAlumnoMalla->fields['creditomax_maa']; ?></td>

			<td><?php echo $rsAlumnoMalla->fields['estado_hac'];?></td>
		<td><?php echo $rsAlumnoMalla->fields['fecestado_hac']; ?></td>
		<td><?php echo $rsAlumnoMalla->fields['nombre_per']; ?></td>
		
		
		<td><?php echo $periodoPre; ?></td>
		<td><?php echo $fechaPre; ?></td>
...
*/
                ...
                if($swRep==0){                                                               
                    ...
                    $arrFin[0]['trimestre'] = $contTrim[$indiceMayor]['trimestre'];
                               ...
                               //return $arrFin;
							   return $contTrim[$indiceMayor]['trimestre'];
                               
                }
    
}

function funPreuniversitario($alumno,$dblink){
$sqlPreU="select nombre_per,fecini_per from alumno as alu,periodo as per,alumnomalla as ama
						where alu.serial_alu=ama.serial_alu
						and ama.serial_per=per.serial_per
						and per.serial_sec=7
						and alu.serial_alu = ".$alumno;	
			////and entregado_rgra like 'SI'						
			$rssqlPreU = $dblink->Execute($sqlPreU);		
			
			
		
	$vector = array($rssqlPreU->fields['nombre_per'],$rssqlPreU->fields['fecini_per']);		
	//echo $vector[0];
	return $vector; 
