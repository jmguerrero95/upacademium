...
where serial_caf in(select serial_caf from cabecerafactura where documento_caf = $fac and tipo_caf = 'NC' and estado_caf<>'ANULADO')
group by serial_dar";
//echo "</p>".$ncRealizadas;
...
//  echo 'fac; '.$serial_caf." param: ".$params[0];
...
$ncRealizadas = "select serial_dar, serial_caf, sum(cantidad_dfa) as cantidad_dfa from detallefactura
where serial_caf in(select serial_caf from cabecerafactura where documento_caf =$params[13] and tipo_caf = 'NC' and estado_caf<>'ANULADO')
group by serial_dar";
//echo "</p>".$ncRealizadas;
...
$creditosporTomar="select dfa.serial_dfa,dfa.cantidad_dfa, ara.portomar_ara, ara.serial_ara, caf.documento_caf
...
if($retcreditosporTomar->RecordCount()>0){
			while(!$retcreditosporTomar->EOF ){
				...
				//INSERTA CREDITOS POR TOMAR EN LA TABLA CREDITOS POR TOMAR				
					if($tipo<>'NC'){					
						$insertarPortomar = "insert into cabeceracreditosportomar(serial_ara, serial_dfa, afavor_cpt,saldo_cpt,fechaabierta_cpt,estado_cpt) values(".$retcreditosporTomar->fields['serial_ara'].",".$retcreditosporTomar->fields['serial_dfa'].",".$retcreditosporTomar->fields['cantidad_dfa'].",".$retcreditosporTomar->fields['cantidad_dfa'].",current_date(),'ABIERTA')";
						$dblink->Execute($insertarPortomar);				
					}else{ //echo "D: ".$retcreditosporTomar->fields['documento_caf'];
					
							
							
							
							$consultarCreditosAfavor="SELECT afavor_cpt,saldo_cpt FROM cabeceracreditosportomar AS cpt WHERE cpt.serial_dfa IN(SELECT	serial_dfa FROM detallefactura AS dfa WHERE serial_caf = ".$params[13].")";																						
						//echo "</p>".$consultarCreditosAfavor;
						
							$retconsultarCreditosAfavor = $dblink->Execute($consultarCreditosAfavor);
							
						
							if($retconsultarCreditosAfavor->RecordCount() > 0){							
								$cantidadAfavor = $retconsultarCreditosAfavor->fields['afavor_cpt'] - $retcreditosporTomar->fields['cantidad_dfa'];
								$cantidadAfavor2 = $retconsultarCreditosAfavor->fields['saldo_cpt'] - $retcreditosporTomar->fields['cantidad_dfa'];
								
								
							//	echo "</p>".$cantidadAfavor;
								
//								$cantidadAfavor = $retconsultarCreditosAfavor->fields['afavor_cpt'] - $retcreditosporTomar->fields['cantidad_dfa'];			
										if($cantidadAfavor >= 0){
											$modificarAfavor = "UPDATE cabeceracreditosportomar SET	afavor_cpt = ".$cantidadAfavor." WHERE serial_dfa IN(SELECT	serial_dfa FROM detallefactura AS dfa WHERE serial_caf = ".$params[13].")";
											$dblink->Execute($modificarAfavor);

																									
										}
										
										else{
										 echo "No se puede Guardar ya que la cantidad créditos a los que va hacer nota de credito es mayor alas que tiene a favor el estudiante.";
										 return;										 
								
								
							}
						}
						/*	while(!$retconsultarCreditosAfavor->EOF ){												
										$cantidadAfavor = $retconsultarCreditosAfavor->fields['afavor_cpt'] - $retcreditosporTomar->fields['cantidad_dfa'];			
										if($cantidadAfavor >= 0){
											$modificarAfavor = "UPDATE cabeceracreditosportomar SET	afavor_cpt = ".$cantidadAfavor." WHERE serial_dfa IN(SELECT	serial_dfa FROM detallefactura AS dfa WHERE serial_caf = ".$retcreditosporTomar->fields['documento_caf'].")";
											$dblink->Execute($modificarAfavor);																										
										}
										/*else{
										 //echo "No se puede Guardar ya que la cantidad créditos a los que va hacer nota de credito es mayor alas que tiene a favor el estudiante.";
								...
								//		$retconsultarCreditosAfavor->MoveNext();
							/*	}		
							}*/						
						 }
