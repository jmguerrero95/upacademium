...
function NOPROFESOR() {
  ...
  $sql="select * from  empleado where prospecto_epl='NO' and estadonomina_epl='ACTIVO' and generarol_epl='SI' AND TIPOEMPLEADO_EPL<>'PROFESOR' 
			and DOCUMENTOIDENTIDAD_EPL in (select DOCUMENTOIDENTIDAD_EPL from empleado where prospecto_epl='NO' and estadonomina_epl='ACTIVO' and TIPOEMPLEADO_EPL='PROFESOR' AND GENERAROL_EPL='SI') and serial_epl=".$serial_epl;
//echo $sql;  
$rs=$dblink->Execute($sql);
...
function CALCULARIMPUESTORENTA_PROFESORES($serial_perrol,$serial_epl){
	...
	$sql="select * from  empleado where prospecto_epl='NO' and estadonomina_epl='ACTIVO' and generarol_epl='SI' AND TIPOEMPLEADO_EPL<>'PROFESOR' 
			and DOCUMENTOIDENTIDAD_EPL in (select DOCUMENTOIDENTIDAD_EPL from empleado where prospecto_epl='NO' and estadonomina_epl='ACTIVO' and TIPOEMPLEADO_EPL='PROFESOR' AND GENERAROL_EPL='SI')".$empleado;
   ...
   while (!$rsempledo_profesor->EOF) {
	   ...
	   $rsprofesor=$dblink->Execute("select * from empleado where DOCUMENTOIDENTIDAD_EPL='".$rsempledo_profesor->fields['DOCUMENTOIDENTIDAD_EPL']."' and
	   								prospecto_epl='NO' and estadonomina_epl='ACTIVO' and TIPOEMPLEADO_EPL='PROFESOR' AND GENERAROL_EPL='SI'");	
...
// proceso de evaluacion de la formula
  ...
  $EmpleadoFechas= "select FECHAINGRESO_EPL,FECHASALIDA_EPL, retfondoreserva_epl from empleado where SERIAL_EPL = ".$empleado;
  $rsEmpleadoFechas=$dblink->Execute($EmpleadoFechas);


  $fechaIngreso=explode('-',$rsEmpleadoFechas->fields['FECHAINGRESO_EPL']);
  $fechaSalida=explode('-',$rsEmpleadoFechas->fields['FECHASALIDA_EPL']);
  $bandera= 0;
  $dia=0;
...
//$rubro_general
		...
		if($dias_diferencia<13){		
				echo "DIAS DIFERENCIA: ".$dias_diferencia."<br>";
				$resultado=0;
				
				echo "<br>pablo ".$rubro_general." / ".$rsEmpleadoFechas->fields['retfondoreserva_epl']."--".$serial_epl;
		}
...
//		 or $rsEmpleadoFechas->fields['retfondoreserva_epl']=='NO'
	...
	if($bandera == 1){
	   if($formula == 'SI(NOPROFESOR(),0,240/12)'){
		$dia=$maxdays-($dia-1);
...
//           echo $SQLCmd;
	   ...
	   $SQLCmd="select * from  empleado where prospecto_epl='NO' and estadonomina_epl='ACTIVO' and generarol_epl='SI' AND (TIPOEMPLEADO_EPL<>'PROFESOR' or  (TIPOEMPLEADO_EPL='PROFESOR' AND GENERAROL_EPL='SI')) order by APELLIDO_EPL";
