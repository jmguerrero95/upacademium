...
function omnisoftContabilizar() {
 ...
 or  '".$params[1]."'  between fechainicio_cid and fechafin_cid) and serial_suc=".$serial_suc."  and  tipo_cid<>'ND'");
 ...
 if($rscierrdia->RecordCount()==0){
   ...
   if($rs_periodo>0){
   	//	echo "entra";
		 ...
		 $rsdebe=$dblink->Execute("select * from detallefactura,cabecerafactura where cabecerafactura.serial_caf=detallefactura.serial_caf and fecha_caf>='".$params[0]."' and fecha_caf<='".$params[1]."' and detallefactura.serial_dar<>0 and serial_plc not in (select serial_plc from plancontable) and serial_suc=".$serial_suc." and cabecerafactura.tipo_caf <> 'ND'");
		 
		 $rshaber=$dblink->Execute("select * from detallefactura,cabecerafactura where cabecerafactura.serial_caf=detallefactura.serial_caf and fecha_caf>='".$params[0]."' and fecha_caf<='".$params[1]."' and detallefactura.serial_dar<>0 and serial_plce not in (select serial_plc from plancontable) and serial_suc=".$serial_suc." and cabecerafactura.tipo_caf <> 'ND'");
		 ...
		 //echo "</p>"."select * from detallefactura,cabecerafactura where cabecerafactura.serial_caf=detallefactura.serial_caf and fecha_caf>='".$params[0]."' and fecha_caf<='".$params[1]."' and detallefactura.serial_dar<>0 and serial_plce not in (select serial_plc from plancontable) and serial_suc=".$serial_suc;		 
		  ...
		  // echo "entra";		   
			...
			//saca los detalles de todas las tablas cabecera y detalle factura
							$SQLCommand="select cabecerafactura.serial_caf,detallefactura.serial_alu,fecha_caf,estado_caf,numero_caf,cliente_caf,cedula_caf,observaciones_caf,serial_dep,valor_aal,valorajuste_aal,detallefactura.serial_plc,detallefactura.serial_plce,cantidad_dfa,descripcion_dar,descuento_dfa,detallefactura.serial_caa,serial_com,concat_ws(' ',apellidopaterno_alu,apellidomaterno_alu,nombre1_alu,nombre2_alu) alumno,codigo_alu, tipo_caf,codigoIdentificacion_alu from cabecerafactura,detallefactura, detallearancel,alumno
		where cabecerafactura.serial_caf=detallefactura.serial_caf  and detallefactura.serial_dar=detallearancel.serial_dar
		and fecha_caf>='".$params[0]."' and fecha_caf<='".$params[1]."' and alumno.serial_alu=cabecerafactura.serial_alu  and cabecerafactura.serial_suc=".$serial_suc."  and estado_caf<>'ANULADO' and cabecerafactura.tipo_caf <> 'ND' ORDER BY tipo_caf, cabecerafactura.serial_caf,fecha_caf asc";						
						 //  echo $SQLCommand."<br>";						 
		...
		/*	 			   $sCuentaRolPagos=omnisoftLeerParametro($dblink,"ctarol");
						   $cuentaRolPagos=omnisoftLeerSerialCuentaContable($dblink,$sCuentaRolPagos);*/
								...
								//////INGRESOS		
								$descripcion=$rsMonto->fields['descripcion_dar']."(".$rsMonto->fields['alumno']."-".$rsMonto->fields['codigoIdentificacion_alu'].")-".$rsMonto->fields['numero_caf'];
								...
								$total_fac=$total_fac+$valor+$rsMonto->fields['descuento_dfa'];
									   ...
									   if($rsMonto->fields['descuento_dfa']!=0){
										...
										$serial_haber=insertarDetalleComprobante($dblink,$serial_com,$rsMonto->fields['serial_plce'],0,$valor_cont,$descripcion,$rsMonto->fields['codigo_alu'].'|'.$rsMonto->fields['numero_caf'],$rsMonto->fields['serial_dep']);
										if ($serial_haber==0) {
												   echo "!Error: ". $dblink->ErrorMsg();
										   return;
										}	   
						   ...
						   $rsMonto->MoveNext();
							...
							//Inserta la cabecera de cierre de dia si no existio facturación
									   $sql_insertcierredia="insert into cierredia (serial_cid,fechainicio_cid,fechafin_cid,valorfac_cid,valorcob_cid,serial_suc,serial_usr,fecha_cid) values (0,'".$params[0]."','".$params[1] ."',0,0,".$serial_suc.",".$serial_usr.",NOW())";
									   $dblink->Execute($sql_insertcierredia);
...
AND cabecerafactura.tipo_caf <> 'ND' 
...
and formacobro.serial_tic not in (select serial_tic from tipocomprobante) and cabecerafactura.serial_suc=".$serial_suc." and cabecerafactura.tipo_caf <> 'ND'");
 ...
 if($rsctas->RecordCount()==0){ 
		...
		and detallerecibo.serial_forc=formacobro.serial_forc
		and cabecerafactura.serial_caf=cabecerarecibo.serial_caf
		and fecha_cre>='".$params[0]."' and fecha_cre<='".$params[1]."'
		and alumno.serial_alu=cabecerafactura.serial_alu
		and cabecerafactura.serial_suc=".$serial_suc." AND cabecerafactura.tipo_caf <> 'ND' 
		...
		/*	 			   $sCuentaRolPagos=omnisoftLeerParametro($dblink,"ctarol");
						   ...
						   $serial_tic_ant=$rsCobros->fields['serial_tic'];
						    ...
						    //ACTUALIZA LA CABECERA CON CO SERIAL DEL COMPROBANTE
									//ACTUALIZA LAS FACTURAS CON EL COMPROBANTE CONTABLE QUE SE CONTABILIZO
									$SQLUpdate_serial="update cabecerarecibo set serial_com=".$serial_com."  where serial_cre=".$rsCobros->fields['serial_cre'];
									$rsupdate=$dblink->Execute($SQLUpdate_serial); 
...
union */
  ...
  select 0,".$serial_cid.",detallerecibo.serial_forc,serial_dre,valor_dre,'PENDIENTE',NOW(),CASE WHEN fecha_dre='0000-00-00' THEN fecha_cre  ELSE fecha_dre END fecha,concat_ws('-',numero_caf,codigoIdentificacion_alu,concat_ws(' ',apellidopaterno_alu,apellidomaterno_alu,nombre1_alu,nombre2_alu)) as descripcion
	from formacobro,cabecerarecibo, detallerecibo,cabecerafactura,alumno
	where 
	cabecerarecibo.serial_cre=detallerecibo.serial_cre
	and cabecerafactura.serial_caf=cabecerarecibo.serial_caf
   ...
   if (strlen($dblink->ErrorMsg())>0){
	   ...
	   return;
		   ...
		   //*****INSERTCION PARA CONTROL DE VOUCHERS DE TARJETAS********/
			   $SQLCommand="select serial_dre,detallerecibo.serial_forc,NOMBRE_FORC,valor_dre,plazo_dre,fecha_dre,comisiontarjetacredito_forc,control_forc ,concat_ws('-',numero_caf,codigoIdentificacion_alu,concat_ws(' ',apellidopaterno_alu,apellidomaterno_alu,nombre1_alu,nombre2_alu)) as descripcion
			from formacobro,cabecerarecibo, detallerecibo,cabecerafactura,alumno
			where 
			cabecerarecibo.serial_cre=detallerecibo.serial_cre
			and cabecerafactura.serial_caf=cabecerarecibo.serial_caf
