...
//            alert(this.sqlCommand);
            ...
            totalPages++;
            document.getElementById('pageLabel').innerHTML = "Página " + (this.grid.pageno + 1) + " de " + totalPages + "| Registros "+this.grid.getTotalRows();
...
*/
 ...
 // add columns
   ...
   this.addColumn = function(aLabel,aColumnName, aWidth,aType,aList,aAlign,aBGColor,aColumnTable) {
...
//          alert(data);
   ...
   this.grid.summary=function(columnCount) {
          ...
          for (var c=0;c<columnCount;c++)
           if ( this.getFields(c).list=='sum' ||this.getFields(c).list=='avg'||this.getFields(c).list=='count') {
            total=0;
            switch(this.getFields(c).list) {
              case 'sum':
               for (var i=0; i < this.getRowCount() ; i++)
